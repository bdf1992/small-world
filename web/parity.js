...
function selectedProfileDisclosure(selected){
  const elemental=selected?.elemental;
  if(!elemental)return '';
  const origins=elemental.profile.contributions.slice().sort((a,b)=>b.share-a.share);
  const readings=Object.values(elemental.clockReading.byTarget).sort((a,b)=>b.score-a.score);
  const strongest=readings[0],weakest=readings[readings.length-1];
  const originRows=origins.map((origin)=>{
    const index=state.elements.indexOf(origin.origin);
    return `<div class="profile-origin"><i style="background:${COLORS[index]}"></i><span>${escapeHtml(origin.origin)}</span><b>${(origin.share*100).toFixed(1)}%</b></div>`;
  }).join('');
  return `<details class="selected-profile-disclosure"><summary><span>M0.7 elemental Profile</span><span class="tag">${origins.length} wheel${origins.length===1?'':'s'} overlaid</span></summary><div class="profile-disclosure-body"><div class="small">Source <span class="mono">${escapeHtml(elemental.source)}</span>. The selected world field is projected through the same canonical Relation Wheel used by the deep instrument.</div><div class="profile-origins">${originRows}</div><div class="profile-clock-read"><b>${escapeHtml(elemental.clockReading.side)} / ${escapeHtml(elemental.clockReading.orientation)}</b> @ <span class="mono">${escapeHtml(elemental.clockReading.at)}</span><br>strongest contextual target <b>${escapeHtml(strongest.target)} ${strongest.score.toFixed(3)}</b><br>weakest <b>${escapeHtml(weakest.target)} ${weakest.score.toFixed(3)}</b></div><a class="profile-deep-link" href="/profile">open full Profile + Crossing instrument ↗</a></div></details>`;
}

function renderSelected(){const selected=state.selected;if(!selected){$('selected').innerHTML='Select a visible cell.';return;}const c=selected.cell;$('selected').innerHTML=`<div class="selected-title">${c.zoneName} · cell #${c.id}</div><div class="small">${c.nucleus?'<span class="tag nucleus">nucleus</span>':''}${c.collision?'<span class="tag collision">front collision</span>':''}${c.resolved?`resolved ${c.element}`:`unresolved · H ${c.entropy.toFixed(3)}`} · neighbors ${c.neighbors.length}<br>root ${escapeHtml(c.root??'—')} · collapse wave ${c.collapseWave??'—'}</div><div class="metric-grid"><div class="metric">initial entropy<b>${Number(c.initialEntropy).toFixed(3)}</b></div><div class="metric">current entropy<b>${Number(c.entropy).toFixed(3)}</b></div></div>${vectorBars(c.probability)}${selectedProfileDisclosure(selected)}<div class="small" style="margin-top:7px"><b>Cyclic seat</b> ${Number(c.cyclicSeat??0).toFixed(2)}<br><b>Static pressure</b> ${topVector(c.externalPressure)}<br><b>Temporal pressure</b> ${c.temporalPressureTotal?topVector(c.temporalPressure):'none'}<br><b>Support updates</b> ${c.supportHits}<br><b>Roots touched</b> ${c.rootsTouched.length?c.rootsTouched.map(escapeHtml).join(' · '):'none yet'}<br><b>Visible recursion</b> ${c.preview.length} deterministic child samples; Dive materializes the conditioned graph.</div>`;}
