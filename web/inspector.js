...
(() => {
  ...
  style.textContent = `
    ...
    .next-wave-evidence{margin-top:8px;padding:7px;border:1px dashed #665472;border-radius:5px;background:#0c0910}.next-wave-evidence strong{color:#e8d6a7}.next-wave-evidence code{font-size:9px;color:#c8becf}.next-wave-evidence .wave-row{padding:3px 0;border-bottom:1px solid #241e29}.next-wave-evidence .wave-row:last-child{border-bottom:0}
  `;
  ...
  function cellObject() {
    ...
    return {
      ...
      possibility:{probability:c.probability,fieldVector:c.fieldVector,nextWave:next?{predictedWave:next.predictedWave,frontier:next.frontier,proposals:next.proposals,collapse:next.collapse,fallback:next.fallback}:null,childPreview:c.preview.map((p)=>`${p.element} @ ${p.point.x.toFixed(2)},${p.point.y.toFixed(2)}`)},
  ...
  function renderWaveDisclosure(){const selected=document.getElementById('selected');if(!selected)return;const existing=selected.querySelector('.next-wave-evidence');if(existing)existing.remove();const html=selectedWaveHtml();if(html)selected.insertAdjacentHTML('beforeend',html);}
  ...
  function refreshInspectorLayer(){bindInspectableChildren();renderWaveDisclosure();renderInspector();drawWaveOverlay();}
